...
class SetViewpointViewController: UIViewController {
    ...
    @IBAction func geometryZoom() {
        self.mapView.setViewpointGeometry(self.griffithParkGeometry, padding: 50) { (finished) in
            
        }
    }
    
    @IBAction func centerScaleZoom() {
        let viewpoint = AGSViewpoint(center: self.londonCoordinate, scale: 40000)
        self.mapView.setViewpoint(viewpoint, duration: 1, curve: AGSAnimationCurve.easeInOutCubic, completion: nil)
    }
    
    @IBAction func zoomInAndOut() {
